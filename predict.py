...
def predict_single(image_path: Path, model: YOLO, conf: float, iou: float, save: bool) -> dict:
    ...
    # Ground truth is derived from the immediate parent folder name
    ground_truth = image_path.parent.name.lower()

    if boxes is None or len(boxes) == 0:
        print(f"  {image_path.name:<40}  →  no detection (conf≥{conf:.0%}) [GT: {ground_truth}]")
        return {"file": image_path.name, "class": None, "conf": None, "correct": False}
    ...
    # Evaluate prediction accuracy against folder name structure
    is_correct = (class_name.lower() == ground_truth)
    match_status = "✓ MATCH" if is_correct else "✗ MISMATCH"

    print(f"  {image_path.name:<40}  →  This is a {class_name} image  "
          f"(conf: {best_conf:.1%})  [{match_status} with '{ground_truth}']")
    ...
    return {"file": image_path.name, "class": class_name, "conf": best_conf, "correct": is_correct}
...
def predict_folder(folder: Path, model: YOLO, conf: float, iou: float, save: bool) -> None:
    ...
    # Accuracy string calculation handles edge conditions smoothly
    accuracy_pct = (correct_cnt / len(results)) * 100 if results else 0.0
    ...
    print(f"  Correct Match  : {correct_cnt} / {len(results)}")
    print(f"  Folder Accuracy: {accuracy_pct:.2f}%") # Adds clean accuracy output
    if class_counts:
        print(f"  Breakdown      :")
